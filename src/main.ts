...
function shell(content: string): string {
  return `<header class="topbar">
    ...
    <nav aria-label="Utility navigation">
      ...
      <span class="connection ${navigator.onLine ? 'connection-online' : 'connection-offline'}" id="connection" role="status"><span></span>${navigator.onLine ? 'Online · data stays local' : 'Offline · work is saved'}</span>
...
function landing(): string {
  return shell(`<main id="main" class="landing">
    ...
    <section class="pricing" aria-labelledby="pricing-title"><div><p class="eyebrow">One-time license</p><h2 id="pricing-title">Use the free queue or remove its limit</h2></div><div><p><strong>$24 once</strong></p><p>Field edition removes the 25-record import limit, adds saved shoots, and enables batch edit patterns. XMP, metadata CSV, and workspace backup exports remain free.</p><button class="primary-button" id="landing-license-button">View Field edition</button></div></section>
  </main>`);
}
